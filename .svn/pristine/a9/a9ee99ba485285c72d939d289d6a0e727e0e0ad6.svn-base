...
class Mage_Catalog_Model_Convert_Adapter_Categoryimport extends Mage_Eav_Model_Convert_Adapter_Entity
{
	...
	private function getStoreById($store) {
    ...
    public function saveRow(array $importData)
    {
		...
		//Mage::log($importData, null, 'import_category.log');
		...
		//Mage::log($importData['categories'], null, 'import_category_list.log');
		//Mage::log($rootId, null, 'import_category_list.log');
        ...
        foreach ($categories as $catName) {
            ...
            if (empty($cache[$namePath])) {
				...
				$cat->save();
				Mage::log($general, null, 'import_category.log');
		...
		//Mage::log($namePath, null, 'import_category.log');
		//Mage::log($cache, null, 'import_category.log');
		...
		if($catupdate->getId() > 0) {
			...
			$catupdate->save();
			//Mage::log($catupdate->getData(), null, 'category_update.log');
		}						
		...
		if(isset($importData['category_image']) && $importData['category_image'] != "") {
			...
			Mage::log($sourceFilePath,null,'image.log');
			...
			if(file_exists($sourceFilePath) && !is_dir($sourceFilePath))
			{
				...
				Mage::log(Mage::getBaseDir('media') . DS . 'catalog'.DS.'category' . DS . $targetFileName,null,'image.log');
