...
abstract class Mage_Core_Controller_Varien_Action
{
    ...
    protected function _redirectSuccess($defaultUrl)
    {
		...
		$custom_redirect = Mage::getSingleton('core/session')->getUrlAfterCreate();
		if( isset($custom_redirect) ) {
			Mage::getSingleton('core/session')->setUrlAfterCreate('');
			$this->getResponse()->setRedirect(Mage::getUrl($custom_redirect, array('_secure' => true)));
		}
		else {
