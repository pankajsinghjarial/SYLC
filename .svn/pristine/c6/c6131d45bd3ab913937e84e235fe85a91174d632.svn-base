	  ...
	  if($version == $CSLH_Config['version']){
       ...
       $UNTRUSTED['action'] = "INSTALL";
        ...
        if($CSLH_Config['version'] == "2.2"){ $installationtype = "upgrade22"; }
         ...
         if($CSLH_Config['version'] == "4.0.0"){ $installationtype = "upgrade400"; }   
         if($CSLH_Config['version'] == "4.0.1"){ $installationtype = "upgrade401"; }   
         if($CSLH_Config['version'] == "4.0.2"){ $installationtype = "upgrade402"; } 
         if($CSLH_Config['version'] == "4.0.3"){ $installationtype = "upgrade403"; } 
         if($CSLH_Config['version'] == "4.0.4"){ $installationtype = "upgrade404"; }                                              
...
if( ($installationtype == "upgrade2125" ) || ($installationtype == "upgrade2126" ) || ($installationtype == "upgrade2127") ){           	 
 ...
 if( ($installationtype == "upgrade2169")  ){
  ...
  if($installationtype == "upgrade315") { 
     ...
     mysql_query($sql,$conn);    
     $sql = "ALTER TABLE `livehelp_config` CHANGE `offset` `offset` FLOAT( 8, 2 ) NOT NULL DEFAULT '0'";
     mysql_query($sql,$conn);       
  ...
  if( ($installationtype == "upgrade323") || ($installationtype == "upgrade324") || ($installationtype == "upgrade325") ) { 
    ...
    $q = "SELECT * FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = '".$UNTRUSTED['database_setup']."' AND TABLE_NAME = 'livehelp_users' AND COLUMN_NAME = 'displayname'";
  ...
  if( ($installationtype == "upgrade333") || ($installationtype == "upgrade334") || 
      ($installationtype == "upgrade335") || ($installationtype == "upgrade336") || 
      ($installationtype == "upgrade337") || ($installationtype == "upgrade338") || 
      ($installationtype == "upgrade339") || ($installationtype == "upgrade3310") || 
      ($installationtype == "upgrade3311") || ($installationtype == "upgrade3312")               
       ) { 

     $sql = "UPDATE livehelp_config set version='$version',maxrequests='99999'";
     mysql_query($sql,$conn);    	   
   
   $installationtype = "upgrade339";   
 }  	   	         
}
...
(5, '', 'layer-Subsilver.gif', '<MAP NAME=myimagemap><AREA HREF=javascript:openLiveHelp() SHAPE=RECT COORDS=0,0,419,216><AREA HREF=javascript:openLiveHelp() SHAPE=RECT COORDS=0,216,319,279><AREA HREF=javascript:closeDHTML() SHAPE=RECT COORDS=326,218,429,280></MAP>', '', 0)";
    ...
    // default website:
   $sql = " INSERT INTO `csyntax_livehelp`.`livehelp_websites` (`id` ,`site_name` ,`site_url` ,`defaultdepartment`)VALUES ('1', 'Your Website', '" . $UNTRUSTED['homepage'] . "', '1')";
   mysql_query($sql,$conn); 
