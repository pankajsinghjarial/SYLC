...
class Mage_Sales_Model_Quote extends Mage_Core_Model_Abstract
{
    ...
    public function addProductAdvanced(Mage_Catalog_Model_Product $product, $request = null, $processMode = null)
    {
        ...
        foreach ($cartCandidates as $candidate) {
			...
			$special_price = $candidate->getSpecialPrice();
			if( !empty($special_price) ) $price = $special_price;
			
			/*
			Mage::log($product_id);
			Mage::log($price);
			Mage::log($special_price);
			*/
			...
			$prd = Mage::getModel('catalog/product')->load($product_id);
			//($product_id==482 || $product_id==483 || $product_id==484)
			
			if( !empty($options) && $prd->getLcSizes() ) {
				foreach($options as $option) {
					$int_data[] = $option;
				}
				//Mage::log($int_data);
				$_price = $price * $int_data[0] * $int_data[1];
